...
//  Qijie Li: 927249
//
//  Problem:		    The Heat Equation onto a 3D heatsink
//						with VTK file output
//
//  Compilation:		g++ Assign2DQ.cpp -o Assign2DQ
//
//  Execution:			./Assign2DQ HeatSink5kElements.grid 	or
//						Assign2DQ HeatSink5kElements.grid 		for windows
//
//  Code samples taken from the sparse matrix class and from the MATLAB code.
/////////////////////////////////////////////////////////////////////////////
...
void	assembleSystem();
void	ConjugateGradient(SparseMatrix& A, double* T, double* b, int M);
...
int	main(int argc, char** argv) {
	...
	if (argc < 2) {
		cout << "Make sure you add HeatSink5kElements.grid" << endl;
		exit(1);
	}

	cout << "Getting the grid" << endl;
	getGrid(argv[1]);
	// Allocate arrays
	T			= new double[N_p];
	s			= new double[N_p];
	...
	cout << "Sparse matrcies are initialized. Assembling the System" << endl;
	assembleSystem();
	A		= M;
	...
	fstream file;
	file.open("assign2.data", ios::out);

	cout << "Computing time marching for loop" << endl;
		...
		if (Boundaries[b].type_=="neumann") {
			for(int f=0; f<Boundaries[b].N_; f++) {
				for(int p=0; p<3; p++) {
					...
					m			= Nodes[p];
					s[m]		+=s_e[p]*Q_base*Gamma[Boundaries[b].indices_[f]]/3;
				}
			}
...
void	ConjugateGradient(SparseMatrix& A, double* T, double* b, int M) {
	...
	cout << ", k = " << k << ", r_norm = " << r_norm << ", T = " << T[0] << endl;
...
void writeData(int l) {	
    ...
    sprintf(fileName, "heatTemp%04d.vtk", l);
