...
% Qijie Li: 927249
%
% Solving the heat equation onto a heatsink with the finite element method
% for the spatial discretization and implicit euler for the temporal
% discretization
%
% Code samples taken from Example15_3, Example22_3 and Tutorial8

%% The function
function Assignment2_DQ_Matlab()

    close all; clear all; clc;

    global T M K s rho Qbase C k Tair h N_p N_f N_e N_b ...
           Points Faces Elements Boundaries Delta_t l;

    [Points, Faces, Elements, Boundaries, N_p, N_f, N_e, N_b] = HeatSink5kElements();
    
    % Simulation parameters
    % x_min needs to be from 0.02 to 0.04 to view the grid correctly
    x_min           =  0.02;
    x_max           =  0.04;
    y_min           =  0.00;
    y_max           =  0.02;
    ...
    assembleSystem();
    % Implicit Euler method for the temporal discretization
    A               = M - Delta_t*K;
    Free            = 1:N_p;

    % Set up animated visualization of results
    figure('WindowStyle', 'docked');
    Solution  = trisurf(Faces, Points(:,1),...
...
function assembleSystem()
    ...
    global M K k s h C rho Tair Qbase N_f N_e N_b Points Faces ...
